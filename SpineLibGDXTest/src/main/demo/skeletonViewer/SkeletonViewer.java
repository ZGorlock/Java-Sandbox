...
public class SkeletonViewer extends ApplicationAdapter {
    ...
    public static final String[] startSuffixes = {"", "-pro", "-ess"};
    
    public static final String[] dataSuffixes = {".json", ".skel"};
    
    public static final String[] endSuffixes = {"", ".txt", ".bytes"};
    
    public static final String[] atlasSuffixes = {".atlas", "-pma.atlas"};
    ...
    boolean loadSkeleton(@Null FileHandle skeletonFile) {
        ...
        if (ui.skinList.getSelected() != null) {
    ...
    public void render() {
        ...
        if (skeleton != null) {
            ...
            if (reloadTimer <= 0) {
                ...
                if (lastModifiedCheck < 0) {
                    ...
                    if (time != 0 && skeletonModified != time) {
                        reloadTimer = reloadDelay;
                    }
                    time = atlas.lastModified();
                    if (time != 0 && atlasModified != 0 && atlasModified != time) {
                        reloadTimer = reloadDelay;
                    }
                }
            } else {
                reloadTimer -= delta;
                if (reloadTimer <= 0 && loadSkeleton(skeletonFile)) {
                    ui.toast("Reloaded.");
                }
            }
            ...
            if (skeleton.getScaleX() == 0) {
                skeleton.setScaleX(0.01f);
            }
            if (skeleton.getScaleY() == 0) {
                skeleton.setScaleY(0.01f);
            }
        ...
        if (state != null) {
            ...
            for (int i = state.getTracks().size - 1; i >= 0; i--) {
                ...
                status(entry);
                status.append("[WHITE]");
                status.append(entry.getAnimation().getName());
                status.append('\n');
            }
    ...
    void status(TrackEntry entry) {
        TrackEntry from = entry.getMixingFrom();
        if (from == null) {
            return;
        }
        status(from);
        status.append(from.getAnimation().getName());
        status.append(' ');
        status.append(Math.min(100, (int) (entry.getMixTime() / entry.getMixDuration() * 100)));
        status.append("% -> ");
    }
    ...
    public void resize(int width, int height) {
        ...
        if (!ui.minimizeButton.isChecked()) {
            ui.window.setHeight(height / uiScale + 8);
        }
    }
                ...
                for (String file : files) {
                    for (String endSuffix : endSuffixes) {
                        for (String dataSuffix : dataSuffixes) {
                            if (file.endsWith(dataSuffix + endSuffix) && skeletonViewer.loadSkeleton(Gdx.files.absolute(file))) {
                                return;
                            }
                        }
                    }
