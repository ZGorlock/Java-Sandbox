/*
 * File:    SkeletonViewerUI.java
 * Package: main.demo.skeletonViewer
 * Author:  Zachary Gill
 */
...
class SkeletonViewerUI {
    ...
    void events() {
        ...
        openButton.addListener(new ChangeListener() {
            public void changed(ChangeEvent event, Actor actor) {
                ...
                if (viewer.loadSkeleton(new FileHandle(new File(dir, name).getAbsolutePath()))) {
                    toast("Loaded.");
                }
            }
        });
        ...
        setupPoseButton.addListener(new ChangeListener() {
            public void changed(ChangeEvent event, Actor actor) {
                if (viewer.skeleton != null) {
                    viewer.skeleton.setToSetupPose();
                }
            }
        });
        bonesSetupPoseButton.addListener(new ChangeListener() {
            public void changed(ChangeEvent event, Actor actor) {
                if (viewer.skeleton != null) {
                    viewer.skeleton.setBonesToSetupPose();
                }
            }
        });
        ...
        loadScaleSlider.addListener(new ChangeListener() {
            public void changed(ChangeEvent event, Actor actor) {
                ...
                if (!loadScaleSlider.isDragging() && viewer.loadSkeleton(viewer.skeletonFile)) {
                    toast("Reloaded.");
                }
        ...
        loadScaleResetButton.addListener(new ChangeListener() {
            public void changed(ChangeEvent event, Actor actor) {
                ...
                if (loadScaleSlider.getValue() == 1) {
                    if (viewer.loadSkeleton(viewer.skeletonFile)) {
                        toast("Reloaded.");
                    }
                } else {
                    loadScaleSlider.setValue(1);
        ...
        xScaleSlider.addListener(new ChangeListener() {
            public void changed(ChangeEvent event, Actor actor) {
                if (xScaleSlider.getValue() == 0) {
                    xScaleSlider.setValue(0.01f);
                }
        ...
        yScaleSlider.addListener(new ChangeListener() {
            public void changed(ChangeEvent event, Actor actor) {
                if (yScaleSlider.getValue() == 0) {
                    yScaleSlider.setValue(0.01f);
                }
        ...
        mixSlider.addListener(new ChangeListener() {
            public void changed(ChangeEvent event, Actor actor) {
                ...
                if (viewer.state != null) {
                    viewer.state.getData().setDefaultMix(mixSlider.getValue());
                }
            }
        });
        ...
        InputListener scrollFocusListener = new InputListener() {
            ...
            public void exit(InputEvent event, float x, float y, int pointer, @Null Actor toActor) {
                if (pointer == -1 && stage.getScrollFocus() == event.getListenerActor()) {
        ...
        ChangeListener trackButtonListener = new ChangeListener() {
            public void changed(ChangeEvent event, Actor actor) {
                ...
                animationList.setSelected(current == null ? null : current.getAnimation().getName());
                ...
                alphaSlider.setValue(current == null ? 1 : current.getAlpha());
        ...
        if (viewer.state != null) {
            ...
            if (current != null) {
                ...
                if (name.equals("<empty>")) {
                    name = current.getNext() == null ? "" : current.getNext().getAnimation().getName();
                }
                prefs.putString("animationName", name);
            }
        }
        if (skinList.getSelected() != null) {
            prefs.putString("skinName", skinList.getSelected());
        }
        prefs.flush();
    }
    ...
    class HigherTrackCheckBox extends CheckBox {
        ...
        protected Drawable getImageDrawable() {
            if (trackButtons.getCheckedIndex() == 0) {
                return getStyle().checkboxOffDisabled;
            }
